...
class AnalysisStream(object):
    ...
    def getBody(self, body):
        ...
        if frame:
            outline[0] = (surface.get_width() / 2,
                          surface.get_height() / 4 * 3)
            for count, (start_limb, end_limb) in enumerate(traverse()):
                ...
                outline[end_limb] = self.get_coords(
                    outline[start_limb], frame[start_limb], length)
    ...
    def get_coords(self, start, radians, length):
        y = math.sin(radians[0]) * length
        x = math.sin(radians[1]) * length
